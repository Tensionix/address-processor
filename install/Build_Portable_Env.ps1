...
Write-Host "======================================================================"
Write-Host "AUDION ADDRESS ALIGNER - BUILD PORTABLE ENV (PS)"
Write-Host "======================================================================"
...
$buildBootstrap = @("setuptools", "wheel", "packaging")
Invoke-Checked -Label "Downloading build bootstrap wheels" -Command {
    & $pythonExe -m pip download --disable-pip-version-check --timeout 120 --retries 12 $buildBootstrap -d $wheelhouseDir
}
Invoke-Checked -Label "Installing build bootstrap wheels" -Command {
    & $pythonExe -m pip install --disable-pip-version-check --no-index --find-links=$wheelhouseDir $buildBootstrap
}
Invoke-Checked -Label "Building wheelhouse" -Command {
    & $pythonExe -m pip download --disable-pip-version-check --no-build-isolation --timeout 120 --retries 12 -r $requirementsFile -d $wheelhouseDir
}
